...
class LAB_FAISS(VectorStore):
    ...
    def __init__(
        ...
        self._normalize_L2 = normalize_L2 or consine_sim
        self._consine_sim = consine_sim
    ...
    def merge_from(self, target: LAB_FAISS) -> None:
    ...
    def __from(
        ...
        **kwargs: Any,
        ...
        faiss = dependable_faiss_import()
        normalize_L2 = normalize_L2 or consine_sim
        index_compute_engine = faiss.IndexFlatIP if consine_sim else faiss.IndexFlatL2
        index = index_compute_engine(len(embeddings[0]))
        vector = np.array(embeddings, dtype=np.float32)
        if normalize_L2:
            faiss.normalize_L2(vector)
        index.add(vector)
